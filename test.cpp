...
void arrayInput         (vector<int>&);
void arrayPrint         (vector<int>&);
int  checkBestPuzzle    (vector<int>&, int, vector<int>&);
bool compareArrays      (vector<int>&, vector<int[]>&);
void move               (char, int, vector<int>&);
void solve              (vector<int>&, vector<vector<int>>&, vector<int>&);
int  tilesOutOfPlace    (vector<int>&);
int  zeroPosition       (vector<int>&);
...
int main ()
{
    ...
    vector<vector<int>> prevPuzzles;                        //Vector of vectors used to track state path to prevent infinite loops where the next best state is equal to the previous state.
    ...
    {
        solve (puzzle, prevPuzzles, pq);             //Call the solve function to find the new puzzle state.
...
void arrayInput (vector<int> &arr)
{
    ...
    {
        cin >> input;                                       //Take in a user input.
        arr.push_back(input);
...
*/
int checkBestPuzzle (vector<int> &tempPuzzle, int bestTilesOutOfPlace, vector<int> &bestPuzzle)
{
    ...
    if (tempTilesOutOfPlace < bestTilesOutOfPlace)          //If temp state has less tiles out of place than the current best.
    {
        ...
        bestPuzzle = tempPuzzle;                            //Make the temp state the new current best state.
    }
...
/*
  ...
  Returns: false if arrays are not a match is found and true if one is
  Description: compares every element of an array to arrays in a vector to check for equality
*/
bool compareArrays (vector<int> &arr, vector<vector<int>> &prevPuzzle)
{
    for (auto previous : prevPuzzle)
        if (previous == arr)
            return true;
    return false;
}
...
*/
void move (char dir, int zeroPos, vector<int> &puzzle)
{
...
*/
void solve (vector<int> &puzzle, vector<vector<int>> &prevPuzzle, vector<int> &pq)
{
    ...
    vector<int> bestPuzzle = puzzle;                        //Used to store the current best puzzle state when calculating possible next states.
    vector<int> tempPuzzle;                                 //Stores a copy of the puzzle for use in calculating states without altering the puzzle.
    ...
    for (int i = 0; i < 4; i++)                             //Loop is used so each possible move (u, d, l, r) is tried once.
    {
        ...
        tempPuzzle = puzzle;                                //Copy puzzle state so it can be worked on without unwanted alterations.
        move (charDir, zeroPos, tempPuzzle);                //Move zero tile in direction specified by previous switch statement.
        //if tempPuzzle is in the pq //if (compareArrays (tempPuzzle, prevPuzzle))         //If the move results in a previous state being achieved (i.e. an infinite loop), don't perform the move.
            continue;
        //? bestTilesOutOfPlace = checkBestPuzzle (tempPuzzle, bestTilesOutOfPlace, bestPuzzle);    //Call checkBestPuzzle to see if the tile move resulted in a better state than other moves. Assign the return to keep track of tiles out of place in the current best state.
        pq.push(tempPuzzle) //Add move to queue
    }

    //? prevPuzzle.push_back(bestPuzzle);                       //Copy the current puzzle as the previous puzzle for comparison later.
    //Sort queue
    puzzle = pq.pop(); //puzzle = front of queue
    //? puzzle = bestPuzzle;                                    //Make the current puzzle state equal to the best puzzle state found by trying every possible move.
...
*/
int tilesOutOfPlace (vector<int> &puzzle)
{
    int x, y, p, q, cost = 0;
...
*/
int zeroPosition (vector<int> &puzzle)
{
